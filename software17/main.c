...
//
	...
	uint32_t temp[16];
	uint32_t cl[16];
	CMD_READ_MOD_HW_accelerator(q_src);
	for(i=0; i<16;i++)
	{
		src[i]    = R2q[i];
		src[16+i] = c[16+i];
	...
	for(i=0; i<16;i++)
	CMD_READ_MOD_HW_accelerator(p_src);
	for(i=0; i<16;i++)
	{
		src[i]    = R2p[i];
		src[16+i] = c[16+i];
	...
	CMD_READ_RSQ_HW_accelerator(src);
	CMD_READ_EXP_HW_accelerator(p_exp_scr);
	...
	CMD_READ_RSQ_HW_accelerator(src);
	CMD_READ_EXP_HW_accelerator(q_exp_scr);
	CMD_COMPUTE_EXP_HW_accelerator();
